    ...
    <div className="space-y-24 pb-20">
      ...
      <section id="home" className="relative pt-16 md:pt-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="col-span-1 lg:col-span-7 space-y-6">
            ...
            <div className="flex flex-wrap gap-4 pt-2">
              ...
              <button
                ...
                className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-800 text-slate-800 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 px-8 py-4 rounded-full font-bold shadow-sm transition-all duration-200 cursor-pointer"
              >
                Learn More
              </button>
            </div>
          ...
          <div className="col-span-1 lg:col-span-5 relative flex justify-center items-center">
            ...
            <div className="relative bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-white/5 p-6 rounded-3xl w-full max-w-md shadow-2xl animate-float-slow">
              <div className="flex justify-between items-center mb-6">
                <div>
                  <h3 className="font-bold text-xs uppercase text-slate-500 dark:text-slate-400">Live Simulator</h3>
                  <p className="text-lg font-black text-slate-800 dark:text-slate-200">Terminal 1 Lot</p>
                </div>
                <div className="h-2.5 w-2.5 rounded-full bg-emerald-500 animate-pulse" />
              </div>

              <div className="grid grid-cols-4 gap-4 p-4 bg-slate-100 dark:bg-slate-950/40 rounded-2xl border border-slate-200 dark:border-slate-800/50">
                {[
                  ...
                  <div key={idx} className="flex flex-col items-center gap-1.5 p-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl">
                    <span className="text-[9px] font-bold text-slate-500 dark:text-slate-400">{slot.id}</span>
                    <div className={`h-4 w-4 rounded-full ${slot.color}`} />
                  </div>
                ))}
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((feature, index) => (
            <motion.div
              key={index}
              whileHover={{ y: -5 }}
              className="bg-white dark:bg-slate-900/60 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 hover:shadow-md transition-all duration-300"
            >
      ...
      <section id="how-it-works" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {steps.map((step, idx) => (
            <div key={idx} className="relative p-6 bg-white dark:bg-slate-900/60 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm">
              <span className="absolute top-4 right-6 text-5xl font-black text-slate-100 dark:text-slate-800 select-none">
                {step.num}
              </span>
      ...
      <section id="future-plans" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {futureFeatures.map((fut, idx) => (
            <div
              key={idx}
              className="p-6 bg-slate-100 dark:bg-slate-900/30 border border-dashed border-slate-300 dark:border-slate-700 rounded-2xl select-none"
            >
              <div className="p-2.5 bg-slate-200 dark:bg-slate-800 rounded-xl w-fit text-slate-500 dark:text-slate-500 mb-4">
                {fut.icon}
              </div>
              <h3 className="text-base font-bold text-slate-600 dark:text-slate-400 mb-1">{fut.title}</h3>
              <p className="text-xs text-slate-500 dark:text-slate-500 leading-relaxed">{fut.desc}</p>
            </div>
          ))}
        </div>
      </section>
